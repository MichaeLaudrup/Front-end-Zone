...
let miFuncion = function () {
  //Declaracion de funcion anonima (no tiene nombre) pero se almacena en una variable cuyo nombre la hace accesible
  /* console.log(
    "Esta es una funcion que solo pertenece al ambito local donde se declara y ademas, solo puede llamarse tras su declaracion, no antes"
  ); */
};

const miFuncion2 = function () {
  console.log(
    "Normalmente en la funciones expresadas (guardadas en una variable) y anonimas se utiliza const en lugar de let"
  );
};
//miFuncion();

//INFINITO NUMERO DE PARAMETROS EN UNA FUNCION (PARAMETROS REST)
function sumarInfinitosNumeros(a, b, ...c) {
  //La c se recibira como un arreglo de elementos
  let resultado = a + b;
  let contador = 2;
...
//Arrow functions
{
  ...
  let casa = {
    nombre: "Amadeus",
    perrito() {
      //Hay que tener mucho cuidado con este tipo de declaraciones
...
/*==================ARREGLOS================*/
{
  ...
  //console.log(array, "Su tama�o es: " + array.length);
  ...
  array.forEach(function (elemento, indice) {
    //en el for each se va iterando sobre los elementos y se envia el elemento a un funcion predefinida por nosotros
...
{
  //Mezclar varios arreglos de manera m�s fac�l con el uso del operador Spread
...
//===DESECTRUCTURACION DE ARRAYS
{
  ...
  const array2 = [1, 2, 3];
  const [x, y, z, w] = array2; //En este caso la posicion 3 de array2 no existe, por lo que w sera undefined
  //se puede usar valores por defecto en caso de no existir esa posicion en el array
  const [i = -1, j = -1, k = -3, l = -5] = array2;
  //console.log(i, j, k, l); // salida : 1 2 3 -5
}
...
//===DESESTRUCTURACION DE OBJETOS
{
  ...
  //console.log(`El alumno  ${nombre} ${apellidos}, con ${edad} a�os, tiene el correo: ${correo}`); //Se aprecia que no se accede al valor de los atributos con persona.nombre, persona.apellidos, si no que se han creado variables
  ...
  //Renombrado de variables que no sean iguales que nombre de atributos
  const { name: nombreRestaurante, location: direccion } = restaurant; //Se han guardado en variables que no tienen que tener equivalencia nominal con los nombres de los atributos del objeto.

  //Valores por defecto
  const { hoursOpen: horariosApertura = "no encontrado", name } = restaurant;
  //console.log(horariosApertura, name); // Salida: no encontrado Classico Italiano;

  //Mutacion de variables
  let a = 111,
    b = 999;
  const obj = { a: 23, b: 7, c: 14 };
  //{a,b} = obj //ERROR: esto no funcionaria
  ({ a, b, c = "no existe", d = "no existe" } = obj); //Con envolverlo en parentesis es suficiente
  //console.log(a, b, c, d); //salida 23, 7, 14, no existe

  //Anidacion de objetos y desestructuracion
  const {
    openingHours: {
      sat: { open, close },
    },

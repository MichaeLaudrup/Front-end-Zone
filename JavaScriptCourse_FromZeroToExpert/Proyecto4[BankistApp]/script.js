...
function calcAndShowBalance(movements) {
  labelBalance.innerHTML = `${movements.reduce(
    (acc, val) => (acc += val),
    0
  )}€`;
}

function displayEstadistics(movements) {
  const inComes = movements
    .filter(item => item > 0)
    .reduce((acc, item) => acc + item, 0);
  const outComes = Math.abs(
    movements.filter(item => item < 0).reduce((acc, item) => acc + item, 0)
  );
...
calcAndShowBalance(movements);
displayEstadistics(movements);
